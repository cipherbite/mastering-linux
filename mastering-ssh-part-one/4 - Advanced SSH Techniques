...
SSH tunneling creates secure channels for transferring data, allowing access to services across firewalls.
...
This makes your local service on port 3000 accessible via port 8080 on public.example.com.
...
This sets up a SOCKS proxy on localhost:1080.
...
SSH Agent Forwarding allows you to use local SSH keys on remote servers without copying the keys to those servers.
